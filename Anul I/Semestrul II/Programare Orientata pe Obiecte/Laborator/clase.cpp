...
public:
    ...
    StupAlbine(char* numeStup, string tipMiere, char culoare, int nrLuni,
        float* cantitateMiereNLuni, bool regina, int nrAlbine); // Constructor parametrizat
    ...
    StupAlbine& operator++();
    StupAlbine operator++ (int);
    StupAlbine operator+(const StupAlbine& obj);
    ...
    const float* getCantitateMiere() const {return this->cantitateMiereNLuni;}
...
float* cantitateMiereNLuni, bool regina, int nrAlbine): idStup(contorId++){
    this->numeStup = new char[strlen(numeStup) + 1];
    strcpy(this->numeStup, numeStup);
    this->tipMiere = tipMiere;
    this->culoare = culoare;
    this->nrLuni = nrLuni;
    this->cantitateMiereNLuni = new float[nrLuni];
    for(int i=0; i<nrLuni; i++)
        this->cantitateMiereNLuni[i] = cantitateMiereNLuni[i];
    this->regina = regina;
}
...
StupAlbine& StupAlbine::operator++() {
    this->nrAlbine++;
    return *this;
}

StupAlbine StupAlbine::operator++ (int){
    StupAlbine aux(*this);
    ///this->nrAlbine++;
    ++*this;
    return aux;
...
StupAlbine StupAlbine::operator+(const StupAlbine& obj){
    StupAlbine aux(*this);
    aux.tipMiere += obj.tipMiere;
    aux.nrAlbine += obj.nrAlbine;
    return aux;
...
ostream& operator <<(ostream& out, const StupAlbine& obj){
    ...
    out << "nrAlbine" << obj.nrAlbine << endl;
    return out;
}
...
int main(){
    ...
    StupAlbine a2(numeStup, "Good Honey", 'g', 3, cantitateMiereNLuni, true, 100);
    cout << a2 << endl << a2++ << endl << a2;
