...
class TaxiAgent:
    def __init__(self, env, gamma=0.8, alpha=1e-1, start_epsilon=1, end_epsilon=1e-2, epsilon_decay=0.999):
...
class ExpectedSARSA_TaxiAgent(TaxiAgent):
    ...
    def updateExperience(self, state, action, reward, next_state, n_episode):
        ...
        # calculate probabilities of actions (pi)
        probs_actions_random = np.array([eps * (1/self.n_action) for _ in range(self.n_action)])
        probs_actions_best = np.array([0 if index != next_action else (1 - eps) for index in range(self.n_action)])
        probs_actions = probs_actions_random + probs_actions_best
        # calculate update equation
        self.q[state][action] = self.q[state][action] + self.alpha * (reward + self.gamma * np.dot(self.q[next_state], probs_actions) - self.q[state][action])
...
if __name__ == "__main__":
    ...
    sarsaAgent = QLearning_TaxiAgent(env, gamma=0.8, alpha=1e-1, start_epsilon=1, end_epsilon=1e-2, epsilon_decay=0.999)
