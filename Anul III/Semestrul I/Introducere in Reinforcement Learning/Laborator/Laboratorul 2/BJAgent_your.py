...
class MonteCarlo_BJAgent(BJAgent):
    ...
    def select_action(self, state, epsilon):
        ...
        if epsilon != None and np.random.rand() < epsilon:
            ...
            # if there is an action value for the state[0] (the player's cards sum),
            # choose the best action for the state (according to policy)
            if state[0] in self.q:
    ...
    def mc_control_glie(self, n_episode=500000, firstVisit=True, update_policy_every=1):
        for t in trange(n_episode):
            ...
            for t in range(len(transitions) - 1, -1, -1):
                ...
                self.n_q[St][At] += 1
                # Remember that with this formula all episodes experiences have equal importance, if you want to forget older episode put a bigger probability
                alpha = (1.0 / self.n_q[St][At])
...
# Just for testing purposes, no need to play but it is useful for debugging purposes
