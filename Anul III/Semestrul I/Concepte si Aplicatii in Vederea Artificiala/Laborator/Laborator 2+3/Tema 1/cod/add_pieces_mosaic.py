...
def get_sorted_distances(patch, mean_color_pieces):
    dist = np.sum((mean_color_pieces - patch) ** 2, axis=1)
    ...
    if params.criterion == 'aleator':
        ...
        for i in range(params.num_pieces_vertical):
            for j in range(params.num_pieces_horizontal):
                ...
                # calculez media patch
                mean_patch = np.mean(patch, axis=(0, 1))
                # calculez distanta dintre patch si flori
                sorted_distances = get_sorted_distances(mean_patch, mean_color_pieces)
                # aleg cea mai buna floare
                index = sorted_distances[0]
                img_mosaic[i*H : (i+1)*H, j*W : (j+1)*W, :] = params.small_images[index].copy()
                # print('Building mosaic %.2f%%' % (100 * (i * params.num_pieces_horizontal + j + 1) / num_pieces))
...
def add_pieces_hexagon(params: Parameters):
    ...
    for i in range(H // 2, bigger_image.shape[0] - H, H):
        for j in range(0, bigger_image.shape[1] - W, W + W//3):
            ...
            # aleg cea mai buna floare
            index = sorted_distances[0]
            img_mosaic[i : i+H, j : j+W, :] = img_mosaic[i : i+H, j : j+W, :] * (1 - mask) + mask * params.small_images[index].copy()
