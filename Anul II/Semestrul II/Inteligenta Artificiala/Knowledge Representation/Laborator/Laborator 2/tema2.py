    ...
    def afis_sol_fisier(self, fisier):
        for nod in self.drum_radacina():
            ...
            if nod.informatie[2] == 1:
                ...
                mis_mal_opus = Graf.NM - mis_mal_curent
                can_mal_opus = Graf.NC - can_mal_curent
                ...
                if parinte is not None:
                    ...
                    fisier.write(
                        f'Barca s-a deplasat de la malul drept la malul stang cu {mis_barca} misionari si {can_barca} canibali.\n')

                # afisez starea
                fisier.write(
                    f'(Stanga:<barca>) {mis_mal_curent} misionari {can_mal_curent} canibali  ......  (Dreapta) {mis_mal_opus} misionari {can_mal_opus} canibali\n\n')

            else:
                # barca pe malul drept
                mis_mal_opus = nod.informatie[0]
                can_mal_opus = nod.informatie[1]
                mis_mal_curent = Graf.NM - mis_mal_opus
                can_mal_curent = Graf.NC - can_mal_opus
                ...
                if parinte is not None:
                    ...
                    # afisez miscarea barcii
                    fisier.write(
                        f'Barca s-a deplasat de la malul stang la malul drept cu {mis_barca} misionari si {can_barca} canibali.\n')

                # afisez starea
                fisier.write(
                    f'(Stanga) {mis_mal_opus} misionari {can_mal_opus} canibali  ......  (Dreapta:<barca>) {mis_mal_curent} misionari {can_mal_curent} canibali\n\n')
...
class Graf:
    ...
    def succesori(self, nod):
        ...
        for mb in range(min_mis_barca, max_mis_barca + 1):
            min_can_barca = 0
            ...
            for cb in range(min_can_barca, max_can_barca + 1):
                ...
                if not test_conditie(mis_mal_curent_nou, can_mal_curent_nou) and nod.informatie[2] == 0:
                    continue
                if not test_conditie(mis_mal_opus_nou, can_mal_opus_nou) and nod.informatie[2] == 1:
                    continue
...
start = (Graf.NM, 0, 1)
# malul final nu poate fi 1 pt ca doar misionarii conduc barca si ar insemna ca un misionar sa fie pe 1
scopuri = [(0, Graf.NC, 0)]
