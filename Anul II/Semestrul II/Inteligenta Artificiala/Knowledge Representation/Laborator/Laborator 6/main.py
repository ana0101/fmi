        ...
        for i in range(InfoJoc.NR_COLOANE):
            for j in range(InfoJoc.NR_COLOANE):
                if self.matr[i][j] == InfoJoc.GOL:
                    info_succesor = copy.deepcopy(self.matr)
                    info_succesor[i][j] = jucator
                    l_mutari.append(InfoJoc(info_succesor))
        return l_mutari
...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == InfoJoc.JMAX:
        ...
        for mutare in stare.mutari_posibile:
            mutare_cu_subarbore = alpha_beta(alpha, beta, mutare)
            if mutare_cu_subarbore.estimare > stare.estimare:
                stare.estimare = mutare_cu_subarbore.estimare
                stare.stare_aleasa = mutare_cu_subarbore
                if alpha < mutare_cu_subarbore.estimare:
                    alpha = mutare_cu_subarbore.estimare
                    if alpha >= beta:
                        return stare
    else:
        stare.estimare = float("inf")
        for mutare in stare.mutari_posibile:
            ...
            if mutare_cu_subarbore.estimare < stare.estimare:
                stare.estimare = mutare_cu_subarbore.estimare
                stare.stare_aleasa = mutare_cu_subarbore
                if beta > mutare_cu_subarbore.estimare:
                    beta = mutare_cu_subarbore.estimare
                    if alpha >= beta:
                        return stare
    return stare
