...
unifya (AtomicRel p ss) (AtomicRel q ts)
    | p == q = unify (zipWith Equ ss ts)
unifya _ _ = Failure
...
varngl :: Goal -> [String]
varngl g = [x | x <- freshvarlist, x `notElem` vargl g]

-- renaming of a clause according to a goal
renclause :: Goal -> Clause -> Clause
renclause g c = substitutecl [Equ (Variable x) (Variable y) | (x, y) <- zip (varcl c) (vargl g)] c

-- substitution for a goal
substitutegl :: [Equ] -> Goal -> Goal
substitutegl es (Goal ats) = Goal (substitutea es <$> ats)
...
evaluatenp p g = concat (map (evaluatenc p g) p)
