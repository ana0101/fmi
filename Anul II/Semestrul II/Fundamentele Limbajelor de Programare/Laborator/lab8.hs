...
annotate_aux (Var x) fvs = (AVar x, fvs)
annotate_aux (Lam x t) (fv : fvs) = let (alt, fvs') = annotate_aux t fvs
                                    in (ALam x fv alt, fvs')
annotate_aux (App t s) (fv : fvs) = let (alt, fvs') = annotate_aux t fvs
                                        (alt', fvs'') = annotate_aux s fvs'
                                    in (AApp alt alt', fvs'')
...
-- auxiliary function for constraints: given an annotated term, a list of fresh variables and a context, 
-- returns the list of equations and the list of remaining fresh variables
...
constraints_aux (AApp t s) (x : (y : xs)) gamma = 
    ...
    in (es1 ++ es2 ++ [Equ (Variable y) (FuncSym "arr" [Variable v, Variable x])], ws)
