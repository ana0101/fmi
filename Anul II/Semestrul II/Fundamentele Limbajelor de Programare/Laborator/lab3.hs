...
evenodd :: Peano -> (Bool, Bool) -- (daca e par, daca e impar)
evenodd = \p ->
    case p of
        Zero -> (True, False)
        Succ p' ->
            let (x, y) = evenodd p' in
                (y, x)

-- evenoddf :: Peano -> (Bool, Bool)
-- evenoddf = undefined

testevenoddf = evenodd (Succ (Succ (Succ Zero))) == (False, True)

denot :: Stmt -> [(String, Int)] -> [(String, Int)]
denot Skip = \s -> s -- (sau id)
denot (AtrE t e) = \s -> s (update t e)
denot (Seq s1 s2) = \s -> denot s2 (denot s1 s)
